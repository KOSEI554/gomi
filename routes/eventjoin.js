...
router.get("/",(req,res) =>{
  ...
  connection.query(query, (err, event_rows) =>{
    ...
    const query = `SELECT participant.*,users.username AS user_name FROM participant INNER JOIN users ON participant.user_id = users.id WHERE participant.event_id = ${eventId}`;
    //console.log(query);
...
router.post("/", (req,res)=>{
  ...
  //console.log(comment);
  const query = 'INSERT INTO participant (user_id, event_id) VALUES ("' + userId + '",'+'"' + eventId + '")';
